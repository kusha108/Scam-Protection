...
function App() {
  ...
  const handleSubmit = async () => {
    ...
    // Simulate API call (replace with your real backend)
    setTimeout(() => {
      const isScam =
    ...
    <div className="app">
      <div className="card">
        ...
        <div className="status">
          {text && <p className="text-sm text-gray-400">{text.length} characters</p>}
          {result && (
            <p className={`result ${result.includes("Scam") ? "danger" : "safe"}`}>
              {result}
            </p>
          )}
        </div>
